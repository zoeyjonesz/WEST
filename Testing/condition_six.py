...
def main():
    ...
    while (system.pressure_threshold('recycling') == 'MOD' and
        ...
        orginal_btb_pressure = system.btb_pressure
        ...
        btb_pressure_change = system.btb_pressure - orginal_btb_pressure
        ...
        if recycle_pressure_change < 0 and btb_pressure_change < 0:
            print(f"Ideal: Recycle tank pressure decreased by {abs(recycle_pressure_change)} units.")
            system.changes_in_tanks(flowrates, flow_rate_index)
            flow_rate_index += 10
        
        elif recycle_pressure_change >= 0 and btb_pressure_change < 0:
            print(f"Condition Change: Recycle tank pressure increased by {abs(recycle_pressure_change)} units, but BTB pressure decreased by {abs(btb_volume_change)} units.")
            print(f"Switching to Condition 9: Recycle tank pressure high, BTA & BTB pressure low.")

        elif recycle_pressure_change < 0 and btb_pressure_change >= 0:
            print(f"Not ideal: Recycle tank pressure decreased by {abs(recycle_pressure_change)} units, but BTB pressure increased by {abs(btb_volume_change)} units.")
            #PURGE THE BTB TANK???
